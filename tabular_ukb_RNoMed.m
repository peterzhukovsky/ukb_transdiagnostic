...
%% subset controls
%%%% remove all participants on meds
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%  medication  %%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ix=(tmpssri==1 | tmptca==1 | tmpmaoi==1 |tmpsnri==1 |tmpatypical==1 );
...
ica_partial(ix,:)=[]; a(ix)=[];s(ix)=[]; clin(ix)=[]; minimal(ix,:)=[];  % cutting data
1-sum(s(strcmp(clin, 'ahc')))/3363 % percent female
...
%%% exclude medicated participants
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%  medication  %%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('D:\Canada_2020\UK_biobank\reports\ordered\medication_ordered.mat')
...
ix=(tmpssri==1 | tmptca==1 | tmpmaoi==1 |tmpsnri==1 |tmpatypical==1 );
ct_ordered(ix,:)=[]; a(ix)=[];s(ix)=[]; clin(ix)=[]; minimal(ix,:)=[];  % cutting data
...
cognitive=cognitive_ordered; cognitive(ix,:)=[];minimal=minimal_ordered; minimal(ix,:)=[]; MDD_prs=MDD_prs_ordered; MDD_prs(ix)=[]; ANX_prs=ANX_prs_ordered; ANX_prs(ix)=[]; PTSD_prs=PTSD_prs_ordered; PTSD_prs(ix)=[];
%%% alphanum trails
T=table(a,s,clin',minimal.x54_2_0, cognitive.tmt_cor); T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=mdl.Coefficients{4:7,:};
%%% fluid intelligence
T=table(a,s,clin', minimal.x54_2_0,cognitive.x20016_2_0); T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});
%%% word pairs - PAL 
T=table(a,s,clin', minimal.x54_2_0,cognitive.x20197_2_0);T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});
%%% symbol digit
T=table(a,s,clin',minimal.x54_2_0, cognitive.x23324_2_0);T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});

%%% pairs matching (Number of incorrect matches in round) - 399.1-2 
T=table(a,s,clin',minimal.x54_2_0, cognitive.x399_2_2+cognitive.x399_2_1); T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});
%%% reaction time 
T=table(a,s,clin',minimal.x54_2_0, clean(cognitive.x20023_2_0, 3));T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});
%%% digit span
T=table(a,s,clin',minimal.x54_2_0, clean(cognitive.x4282_2_0,3));T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});
%%% Raven's matrices
T=table(a,s,clin',minimal.x54_2_0, cognitive.x6373_2_0);T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});
%%% Tower of london - nr of figures done 21004
T=table(a,s,clin', minimal.x54_2_0, cognitive.x21004_2_0);T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});

%%% neuroticism
T=table(a,s,clin', minimal.x54_2_0,  cognitive.x20127_0_0);T(isnan(T.Var5),:)=[];T.Var5=zscore(T.Var5);
mdl = fitlm(T,'Var5~a+s+a^2+a*s+Var3+Var4')
tmp=horzcat(tmp, mdl.Coefficients{4:7,:});
